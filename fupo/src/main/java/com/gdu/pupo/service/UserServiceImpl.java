...
public class UserServiceImpl implements UserService {
	...
	public Map<String, Object> sendAuthCode(String email) {
		...
		sendMail.setSubject("[FullPowerSalad 인증메일 입니다.]");
		javaMailUtil.sendJavaMail(email, "[FullPowerSalad] 인증메일 입니다.", "<h2>FullPowerSalad 회원가입 메일 인증</h2><br><br>FullPowerSalad에 오신 것을 환영합니다!<br>아래 '이메일 인증 코드'를 확인해주세요!<br>인증번호는 <strong>" + authCode + "</strong>입니다.");
	...
	public void findPw(HttpServletRequest request, HttpServletResponse response) {
			...
			try {
				...
				if("0".equals(cntUserYn)) { //유저가 존재하지 않을 때
					...
					try {
						MailHandler sendMail = new MailHandler();
						sendMail.setSubject("[FullPowerSalad 임시비밀번호 발급 메일입니다.]");
						javaMailUtil.sendJavaMail(email, "[FullPowerSalad] 임시비밀번호 발급 메일 입니다.", "<h2>FullPowerSalad 임시비밀번호 발급 메일</h2><br><br>아래 '임시비밀번호'를 확인해주세요!<br>임시비밀번호는 <strong>" + tempPw + "</strong>입니다.");
